...
export const vehicleTypeDataGet = createAsyncThunk(
  VEHICLE_TYPE,
  async ({ service_id, service_category_id }: { service_id: number, service_category_id: number }) => {
    const response = await vehicleTypeServices.vehicleTypes(service_id, service_category_id);
