...
export const vehicleTypes = async (service_id: number, service_category_id: number) => {
  return GET_API( `${vehicleType}?service_id=${service_id}&service_category_id=${service_category_id}`)
  .then((res) => {
    return res;
  })
  .catch((e) => {
