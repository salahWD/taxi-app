        ...
        <View style={styles.headerMain}>
            <View style={styles.headerMargin}>
                <View style={[styles.headerAlign, { flexDirection: viewRtlStyle }]}>
                    ...
                    <TouchableOpacity style={styles.notificationIcon} activeOpacity={0.5} onPress={() => navigate('Notification')}>
                        <Icons.Notification color={appColors.white} />
                    </TouchableOpacity>
                </View>

                <View style={[styles.switchContainer, { backgroundColor: colors.card, flexDirection: viewRtlStyle }]}>
                    <View style={{ flexDirection: viewRtlStyle ,gap:3}}>
                        <Text style={[styles.valueTitle, { color: colors.text }]}>Available For Ride</Text>
                        <Text style={[styles.valueTitle, { color: isOn ? appColors.price: appColors.red}]}>
                            {isOn ? ' ( Online )' : ' ( Offline )'}
                        </Text>
                    </View>
                        <SwitchToggle
