...
export function RenderRideItem({ item, colors, }: RenderRideItemProps) {
    ...
    return (
        <TouchableOpacity onPress={() => gotoDetail(item)} style={styles.main} activeOpacity={0.8}>
            <View style={styles.mainContainer}>
                <View style={[styles.card, { borderColor: colors.border, backgroundColor: colors.card }]}>
                    <View style={[styles.cardTop, { flexDirection: viewRtlStyle }]}>
                        ...
                        <View style={[styles.iconContain, { backgroundColor: colors.background }]}>
                            {icon}
                        </View>
                    </View>
                    <View style={[styles.cardBottom, { flexDirection: viewRtlStyle }]}>
                        <View>
                            <Text style={[styles.title, { color: colors.text }, { textAlign: textRtlStyle }]}>{item.title}</Text>
                        </View>
